...
export async function runAgentLoop(prompt: string, tools: CodingFleetTool[], maxIterations = 6, authToken?: string): Promise<AgentRunResult> {
...
Never claim an external action succeeded without evidence.`;
  ...
  for (let iteration = 0; iteration < Math.max(1, Math.min(maxIterations, 8)); iteration += 1) {
    ...
    const result = await callWithFallback(currentPrompt, tools, undefined, undefined, authToken);
    if (!result.ok) {
