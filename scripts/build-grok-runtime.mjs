...
let cargo = true;
try { execFileSync("cargo", ["--version"], { stdio: "ignore" }); } catch { cargo = false; }
if (!cargo) {
...
execFileSync("cargo", ["build", "--release", "-p", "xai-grok-pager-bin"], { cwd: join(root, "vendor", "grok-build"), stdio: "inherit" });
mkdirSync(dirname(output), { recursive: true });
const built = join(root, "vendor", "grok-build", "target", "release", process.platform === "win32" ? "xai-grok-pager.exe" : "xai-grok-pager");
